...
<table class="table livesearch">
	...
	<?php $count=0; ?>
	<?php if(is_array($main_10_week) && $main_10_week){ ?>
	<?php foreach ($main_10_week as $key => $val): ?>
		<tr>
			<td>
			...
			<td>
				...
				<div class="btn-group">
					<!-- <form action='<?php echo base_url();?>package/update_10_week' method='post' name='packageedit'>
					<button class="btn btn-primary" value="<?php echo $val["packageid"]; ?>" name="agentidedit">Edit</button>
					</form> -->
					<form action='<?php echo base_url();?>package/delete_10_week' method='post' name='packagedelete'>
