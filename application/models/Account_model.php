...
class Account_model extends CI_Model{
    ...
    public function getuserdata(){
        ...
        $this->db->select('a.accountid , SUM(a.totalamount), SUM(pa.payment), a.refid, a.readytorun, a.customerid, c.customername, c.wechatname , a.oriamount, a.amount, MIN(a.datee), a.interest, a.duedate, a.packageid, ag.agentname, p.packagetypename, MIN(a.status)');
        ...
        $this->db->join('payment pa', 'a.accountid = pa.accountid', 'left');
    ...
    public function getbaddebtuserdata(){
        // Run the query
        // $this->db->distinct('a.refid');
        $this->db->select('b.accountid, a.accountid ,SUM(pa.payment), SUM(a.totalamount) ,a.refid, a.customerid,c.wechatname, c.customername, a.oriamount, a.amount, a.datee, a.interest, MAX(a.duedate), a.packageid, ag.agentname, p.packagetypename, a.guarantyitem');
  ...
  public function getbaddebtaccid(){
    ...
    public function getuserdatamodal($data){
        ...
        $this->db->select('a.accountid, a.totalamount, a.refid, a.oriamount, a.customerid, c.customername,c.wechatname, a.amount, a.datee, a.interest, a.duedate, a.packageid, ag.agentname, ag.agentid, p.packagetypename, a.guarantyitem, a.pullnextperiod');
        $this->db->from('account a');
        $this->db->join('customer c', 'a.customerid = c.customerid', 'left');
        $this->db->join('agent ag', 'a.agentid = ag.agentid', 'left');
        $this->db->join('packagetype p', 'a.packagetypeid = p.packagetypeid', 'left');
    ...
    public function getuserdatainsertpackage_5days_4week(){
     ...
     public function getuserdatainsertpackage_20_4week_5days(){
        // Run the query
        $this->db->select('*');
        ///////////////Combo of User Indentity///////////////////
        $company_identity = $this->session->userdata('adminid');
    ...
    public function getuserdatainsertpackage_10_5days2(){
        // Run the query
        $this->db->select('*');
        ///////////////Combo of User Indentity///////////////////
        $company_identity = $this->session->userdata('adminid');
    ...
    public function interest_30_4week()
    {
        ...
        foreach ($payment_info as $key => $value) {
            ...
            if ($days>0 && $date2<$date1 ) 
            {
                ...
                if($packagename == "package_30_4week" && $status !=="closed" && $status !=="stop" )
                {
                    $total_interest = 0;
                ...
                }elseif($packagename == "package_manual_5days_4week" && $status !=="closed" && $status !=="stop" )
                {
                    $total_interest = 0;
                    for ($i=1; $i <$days+1 ; $i++) {
                        $date_eachday = strtotime("+ ".$i." days", $due_date); //duedate +x days
                ...
                elseif($packagename == "package_20_4week_5days" && $status !=="closed" && $status !=="stop" )
                {
                    $total_interest = 0;
                    for ($i=1; $i <$days+1 ; $i++) {
                        $date_eachday = strtotime("+ ".$i." days", $due_date); //duedate +x days
                ...
                elseif ($packagename == "package_25_month" && $status !=="closed" && $status !=="stop"  )
                {   
                    //日期小过duedate的全部加起来
                    $payment_amount_date_less_than_duedate = 0;
                    $payment_amount_date_larger_than_duedate = 0;
            ...
            if ($packagename == "package_20_week"  && $status !=="closed" && $status !=="stop" )
            ...
            if($packagename == "package_15_week"  && $status !=="closed" && $status !=="stop" )
...
if($packagename == "package_10_week"  && $status !=="closed" && $status !=="stop" )
            ...
            if($packagename == "package_10_5days"  && $status !=="closed" && $status !=="stop" )
...
if($packagename == "package_10_5days2"  && $status !=="closed" && $status !=="stop" )
    ...
    public function count_total_amount()
   {
        ...
        {
            if ($val['packagetypename'] == "package_25_month"|| $val['packagetypename'] == "package_20_week" || $val['packagetypename'] == "package_15_week" || $val['packagetypename'] == "package_10_week" || $val['packagetypename'] == "package_15_5days" || $val['packagetypename'] == "package_10_5days" || $val['packagetypename'] == "package_10_5days2") 
            {
                //do nothing
                $duedate = $val['duedate'];
                $today = date("Y-m-d");
